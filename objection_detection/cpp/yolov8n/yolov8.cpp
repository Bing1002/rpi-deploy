...
static int detect_yolov8(ncnn::Net& model, const cv::Mat &bgr, std::vector<Object>& objects)
{
...
int webcam_demo(ncnn::Net& model, int cam_id)
{
    return 0;
...
int image_demo(ncnn::Net& model, const char* imagepath)
{
    // opencv load image
...
int video_demo(ncnn::Net& model, const char* videopath)
{
...
int benchmar(ncnn::Net& model)
{
    ...
    for (int i = 0; i < warm_up + loop_num; i++) {
        ...
        if ((i + 1) % (10 + warm_up) == 0) { // Print every 10th iteration
...
int main(int argc, char** argv) {
    if (argc != 3) {
        fprintf(stderr, "Usage: %s [mode] [path] \n For webcam: mode=0, path=cam_id; \n For image demo: model=1, path=xxx/xxx/*.jpg; \n For video demo: model=2, path=xxx.mp4; \n For benchmark: mode=3, path=0.\n ", argv[0]);
        return -1;
    }
    ...
    case 0: {
        ...
        webcam_demo(yolov8, cam_id);
        break;
    }
    case 1: {
        const char* imagepath = argv[2];
        image_demo(yolov8, imagepath);
        break; 
    }
    case 2: {
        const char* videopath = argv[2];
        video_demo(yolov8, videopath);
        break;
    }
    case 3: {
        benchmar(yolov8);
